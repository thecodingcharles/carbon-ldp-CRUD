...
var DeltaCreator = (function () {
    ...
    DeltaCreator.prototype.addResource = function (id, previousResource, currentResource) {
        ...
        new Set([
            ...
            if (definition && definition.containerType === ContainerType_1.ContainerType.LIST && Utils_2._isExistingValue(oldValue)) {
                var listUpdates = [];
                if (!Utils_2._isExistingValue(newValue)) {
                    deleteTriples.addProperty(new tokens_1.PropertyToken(predicateURI).addObject(new tokens_1.CollectionToken()));
                ...
                var addValues = function (objects, triple) {
                    if (!objects.length)
                        return;
                    var property = new tokens_1.PropertyToken(predicateURI);
    ...
    DeltaCreator.prototype.__expandValues = function (values, schema, definition) {
        ...
        return values
            ...
            .filter(Utils_2._isExistingValue);
    };
    ...
    DeltaCreator.prototype.__expandLiteral = function (value, schema, definition) {
        ...
        if (type !== XSD_1.XSD.string)
            return new tokens_1.RDFLiteralToken(value, this.__compactIRI(schema, type));
        if (definition && typeof definition.language === "string")
            return new tokens_1.RDFLiteralToken(value, new tokens_1.LanguageToken(definition.language));
        return new tokens_1.LiteralToken(value);
    };
    DeltaCreator.prototype.__compactIRI = function (schema, iri) {
        ...
        if (!matchPrefix)
            return new tokens_1.IRIRefToken(iri);
    ...
    DeltaCreator.prototype.__addPrefixFrom = function (object, schema) {
        ...
        this.prefixesMap.set(namespace, new Tokens_1.PrefixToken(namespace, new tokens_1.IRIRefToken(iri)));
    };
    return DeltaCreator;
}());
